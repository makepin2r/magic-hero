...
namespace Lean.Gui
{
	...
	public class LeanJoystick : LeanSelectable, IPointerDownHandler, IPointerUpHandler, IDragHandler
	{
		...
		private bool isTouch; // 움직일 수 있는 상태인지 체크 (조이스틱 터치 시작, 드래그)
		private Vector3 movePos; // 플레이어가 움직일 위치
        ...
        {
			Vector2 value = eventData.position.normalized;

			movePos = new Vector3(value.x * moveSpeed * Time.deltaTime, 0f, value.y * moveSpeed * Time.deltaTime);
			Debug.Log(movePos);
        }
    }
}
